   ...
   <ElTabGroup>
    <Container className="flex flex-col gap-16">
     <div className="flex flex-col items-center gap-6">
      ...
      <ElTabList className="flex items-center gap-1 rounded-full bg-olive-950/5 p-1">
       {options.map((option) => (
        <button
         key={option}
         type="button"
         className="rounded-full px-4 py-1 text-sm/7 font-medium text-olive-950 aria-selected:bg-olive-950 aria-selected:text-white"
        >
         {option}
        </button>
       ))}
